...
namespace Nefta.AdSdk.Editor
{
    public class UnityWebViewPostprocessBuild : IPostGenerateGradleAndroidProject
    {
        ...
        public static void OnPostprocessBuild(BuildTarget buildTarget, string path)
        {
            ...
            {
                var method =
 type.GetMethod("AddBuildProperty", new Type[]{typeof(string), typeof(string), typeof(string)});
    ...
    internal class AndroidManifest : AndroidXmlDocument
    {
        ...
        internal bool SetUsesCleartextTraffic(bool enabled)
        {
            ...
            bool changed = false;
            if (ApplicationElement.GetAttribute("usesCleartextTraffic", AndroidXmlNamespace) !=
                ((enabled) ? "true" : "false"))
            {
                ApplicationElement.SetAttribute("usesCleartextTraffic", AndroidXmlNamespace,
                    (enabled) ? "true" : "false");
                changed = true;
            }
        ...
        internal bool AddFileProvider(string basePath)
        {
            ...
            if (SelectNodes("/manifest/application/provider[@android:authorities='" + authorities + "']", nsMgr)
                    .Count == 0)
            {
        ...
        internal bool AddCamera()
        {
            ...
            if (SelectNodes("/manifest/uses-permission[@android:name='android.permission.ACCESS_MEDIA_LOCATION']",
                    nsMgr).Count == 0)
            {
                var elem = CreateElement("uses-permission");
            ...
            if (SelectNodes("/manifest/queries/intent/action[@android:name='android.media.action.IMAGE_CAPTURE']",
                    nsMgr).Count == 0)
            {
        ...
        {
            bool changed = false;
            if (SelectNodes("/manifest/uses-permission[@android:name='android.permission.MICROPHONE']", nsMgr).Count ==
                0)
            {
                var elem = CreateElement("uses-permission");
            ...
            if (SelectNodes("/manifest/uses-permission[@android:name='android.permission.MODIFY_AUDIO_SETTINGS']",
                    nsMgr).Count == 0)
            {
                var elem = CreateElement("uses-permission");
            ...
            if (SelectNodes("/manifest/uses-permission[@android:name='android.permission.RECORD_AUDIO']", nsMgr)
                    .Count == 0)
            {
                var elem = CreateElement("uses-permission");

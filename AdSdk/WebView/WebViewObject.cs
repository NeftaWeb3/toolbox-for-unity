...
namespace Nefta.AdSdk
{
    ...
    public class WebViewObject : MonoBehaviour
    {
        ...
        private Callback onJS;
        private Callback onError;
        private Callback onHttpError;
        private Callback onStarted;
        private Action<AdPlacement> onLoaded;
        private Callback onHooked;
        private Callback onCookies;
        ...
        private byte[] textureDataBuffer;
        private int bitmapRefreshCycle = 1;
#elif UNITY_IPHONE
        private IntPtr webView;
#elif UNITY_ANDROID
        ...
        private int mWindowVisibleDisplayFrameHeight;
        private float mResumedTimestamp;
    #if UNITYWEBVIEW_ANDROID_ENABLE_NAVIGATOR_ONLINE
        private float androidNetworkReachabilityCheckT0 = -1.0f;

        private NetworkReachability? androidNetworkReachability0 = null;
    #endif
        ...
        private void Update()
        {
            ...
            if (mResumedTimestamp != 0.0f && Time.realtimeSinceStartup - mResumedTimestamp > 0.5f)
            {
                ...
                webView.Call("SetVisibility", visibility);
            }
            for (;;) {
                if (webView == null)
                    break;
     ...
     UnityClass.GetStatic<AndroidJavaObject>("currentActivity").Get<AndroidJavaObject>("mUnityPlayer").Call<AndroidJavaObject>("getView");
        ...
        private Vector3Int HandleInput()
        {
            Vector3Int clickPosition = Vector3Int.back;
            ...
            for (var i = 0; i < Input.touchCount; i++)
            {
                ...
                foreach (var existingTouch in touches)
                {
                    if (touch.fingerId == existingTouch.id)
                    {
                        isNewTouch = false;
                ...
                if (isNewTouch)
                {
                    touches.Add(new Touch() { id = touch.fingerId, position = touch.position });
                }
            }
           
            for (var i = touches.Count - 1; i >= 0; i--)
            {
                if (touches[i].position.z < 0)
                {
                    clickPosition = new Vector3Int((int)touches[i].position.x, (int)touches[i].position.z, 3);
                    touches.RemoveAt(i);
                }
            }
            ...
            {
                if (previousMousePosition != Vector3.back)
                {
                    clickPosition = new Vector3Int((int) previousMousePosition.x, (int) previousMousePosition.y, 3);
                }
                previousMousePosition = Vector3.back;
            }
            
            if (clickPosition.z >= 0 && rect.Contains(clickPosition))
            {
                clickPosition.x -= (int) rect.x;
                clickPosition.y -= (int) rect.y;
                OnClick?.Invoke();
            }
        ...
        [DllImport("WebView")]
        private static extern void _CWebViewPlugin_SendMouseEvent(IntPtr instance, int x, int y, float deltaY,
            int mouseState);
    ...
    private static extern void _gree_unity_webview_destroy(string name);
            ...
            bool separated = false)
        {
...
#elif UNITY_EDITOR_OSX || UNITY_STANDALONE_OSX
            {
                ...
                var info = System.IO.File.ReadAllText(uri.LocalPath + "Contents/Info.plist");
                if (System.Text.RegularExpressions.Regex.IsMatch(info, @"<key>CFBundleGetInfoString</key>\s*<string>Unity version [5-9]\.[3-9]")
                    && !System.Text.RegularExpressions.Regex.IsMatch(info,
...
#elif UNITY_IPHONE
            webView =
     _CWebViewPlugin_Init(name, transparent, zoom, ua, enableWKWebView, wkContentMode, wkAllowsLinkPreview, wkAllowsBackForwardNavigationGestures, radius);
#elif UNITY_ANDROID
     ...
     UnityClass.GetStatic<AndroidJavaObject>("currentActivity").Get<AndroidJavaObject>("mUnityPlayer").Call<AndroidJavaObject>("getView");
        ...
        {
            NeftaCore.Log($"LoadURL {url}");
            if (string.IsNullOrEmpty(url))
            {
                return;
            }
#if UNITY_WEBGLUN || UNITY_WEBPLAYER || ITY_EDITOR_WIN || UNITY_STANDALONE_WIN || UNITY_EDITOR_LINUX
            //TODO: UNSUPPORTED
#elif UNITY_EDITOR_OSX || UNITY_STANDALONE_OSX || UNITY_IPHONE
            if (webView == IntPtr.Zero)
                return;
        ...
        public void LoadAdPlacement(AdPlacement placement, string html)
        {
            ...
            _CWebViewPlugin_LoadHTML(webView, html, "");
#elif UNITY_ANDROID
            if (webView == null)
            {
                return;
            }
            webView.Call("LoadHTML", html, "");
#endif
        }
        ...
        {
            if (onLoaded != null && _adPlacement != null)
            {
                onLoaded(_adPlacement);
            }
        }
        ...
        private void Update() {
            ...
            var clickPosition = HandleInput();
            if (clickPosition.z >= 0)
            {
                //clickPosition.x -= (int) rect.x;
                //clickPosition.y -= (int) rect.y;
                //_CWebViewPlugin_SendMouseEvent(webView, clickPosition.x, clickPosition.y, 0, clickPosition.z);   
            }
